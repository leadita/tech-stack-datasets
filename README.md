# Live Technographic Dataset

> 📊 Open-source dataset of 55.5M+ companies and their technology stacks, updated daily.

A comprehensive, freely accessible dataset containing real-world company data paired with their technology choices. Perfect for sales teams, marketers, data scientists, and product analysts.

## 📂 Browse by Technology

### A/B Testing & Conversion Optimization
- [Companies using Optimizely](./leads/companies-using-optimizely) (4,967 companies)
- [Companies using VWO](./leads/companies-using-vwo) (9,750 companies)
- [Companies using AB Tasty](./leads/companies-using-ab-tasty) (768 companies)
- [Companies using Dynamic Yield](./leads/companies-using-dynamic-yield) (265 companies)
- [Companies using Monetate](./leads/companies-using-monetate) (573 companies)
- [Companies using Convert](./leads/companies-using-convert) (106 companies)
- [Companies using Kameleoon](./leads/companies-using-kameleoon) (268 companies)
- [Companies using Mutiny](./leads/companies-using-mutiny) (11 companies)
- [Companies using Intellimize](./leads/companies-using-intellimize) (3,414 companies)

### Accessibility (a11y)
- [Companies using accessiBe](./leads/companies-using-accessibe) (19,103 companies)
- [Companies using AudioEye](./leads/companies-using-audioeye) (17,100 companies)
- [Companies using Monsido](./leads/companies-using-monsido) (799 companies)
- [Companies using Texthelp](./leads/companies-using-texthelp) (170 companies)
- [Companies using WP Accessibility](./leads/companies-using-wp-accessibility) (6,564 companies)
- [Companies using Equally AI](./leads/companies-using-equally-ai) (449 companies)

### Accounting & Finance
- [Companies using QuickBooks Online](./leads/companies-using-quickbooks-online) (359 companies)
- [Companies using Xero](./leads/companies-using-xero) (1,352 companies)
- [Companies using FreshBooks](./leads/companies-using-freshbooks) (129 companies)
- [Companies using Sage Intacct](./leads/companies-using-sage-intacct) (900 companies)
- [Companies using Zoho Books](./leads/companies-using-zoho-books) (76 companies)
- [Companies using Expensify](./leads/companies-using-expensify) (34 companies)
- [Companies using Tableau](./leads/companies-using-tableau) (905 companies)
- [Companies using Power BI](./leads/companies-using-power-bi) (825 companies)
- [Companies using Dext](./leads/companies-using-dext) (194 companies)

### Payments
- [Companies using Stripe](./leads/companies-using-stripe) (70,939 companies)
- [Companies using PayPal](./leads/companies-using-paypal) (67,668 companies)
- [Companies using Adyen](./leads/companies-using-adyen) (229 companies)
- [Companies using Braintree](./leads/companies-using-braintree) (2,688 companies)
- [Companies using Authorize.net](./leads/companies-using-authorize.net) (114,691 companies)
- [Companies using Klarna](./leads/companies-using-klarna) (5,320 companies)
- [Companies using Affirm](./leads/companies-using-affirm) (2,682 companies)
- [Companies using Afterpay](./leads/companies-using-afterpay) (6,208 companies)
- [Companies using Apple Pay JS](./leads/companies-using-apple-pay-js) (453 companies)
- [Companies using Google Pay](./leads/companies-using-google-pay) (3,388 companies)

### Advertising
- [Companies using Google Adsense](./leads/companies-using-google-adsense) (92,353 companies)
- [Companies using Amazon Advertising](./leads/companies-using-amazon-advertising) (200 companies)
- [Companies using AppNexus](./leads/companies-using-appnexus) (5,871 companies)
- [Companies using Taboola](./leads/companies-using-taboola) (13,526 companies)
- [Companies using Outbrain](./leads/companies-using-outbrain) (3,088 companies)
- [Companies using Criteo](./leads/companies-using-criteo) (4,052 companies)
- [Companies using Media.net](./leads/companies-using-media.net) (10,746 companies)
- [Companies using AdRoll](./leads/companies-using-adroll) (10,119 companies)
- [Companies using The Trade Desk](./leads/companies-using-the-trade-desk) (197 companies)
- [Companies using Advertising.com](./leads/companies-using-advertising.com) (4,408 companies)

### Affiliate Marketing
- [Companies using Impact](./leads/companies-using-impact) (3,578 companies)
- [Companies using AWIN](./leads/companies-using-awin) (743 companies)
- [Companies using Commission Junction](./leads/companies-using-commission-junction) (458 companies)
- [Companies using ShareASale](./leads/companies-using-shareasale) (4,044 companies)
- [Companies using Rakuten Affiliate](./leads/companies-using-rakuten-affiliate) (5,583 companies)
- [Companies using PartnerStack](./leads/companies-using-partnerstack) (739 companies)
- [Companies using TUNE](./leads/companies-using-tune) (2,441 companies)
- [Companies using Everflow](./leads/companies-using-everflow) (359 companies)
- [Companies using Refersion](./leads/companies-using-refersion) (1,233 companies)

### Analytics
- [Companies using Google Analytics](./leads/companies-using-google-analytics) (2,758,492 companies)
- [Companies using Mixpanel](./leads/companies-using-mixpanel) (7,288 companies)
- [Companies using Amplitude](./leads/companies-using-amplitude) (7,620 companies)
- [Companies using Heap](./leads/companies-using-heap) (2,316 companies)
- [Companies using Hotjar](./leads/companies-using-hotjar) (58,226 companies)
- [Companies using FullStory](./leads/companies-using-fullstory) (1,849 companies)
- [Companies using Segment](./leads/companies-using-segment) (9,701 companies)
- [Companies using Matomo](./leads/companies-using-matomo) (45,048 companies)
- [Companies using CrazyEgg](./leads/companies-using-crazyegg) (8,886 companies)

### Artificial Intelligence
- [Companies using OpenAI](./leads/companies-using-openai) (7,428 companies)
- [Companies using Intercom](./leads/companies-using-intercom) (14,287 companies)
- [Companies using Algolia](./leads/companies-using-algolia) (2,499 companies)
- [Companies using Ada Support](./leads/companies-using-ada-support) (162 companies)
- [Companies using Dialogflow](./leads/companies-using-dialogflow) (350 companies)
- [Companies using Botpress](./leads/companies-using-botpress) (1,115 companies)
- [Companies using Freshworks Bot](./leads/companies-using-freshworks-bot) (1,440 companies)
- [Companies using LandBot](./leads/companies-using-landbot) (541 companies)
- [Companies using Yellow Messenger](./leads/companies-using-yellow-messenger) (139 companies)

### Automotive
- [Companies using Dealer.com](./leads/companies-using-dealer.com) (2,115 companies)
- [Companies using CDK Global](./leads/companies-using-cdk-global) (19 companies)
- [Companies using DealerOn](./leads/companies-using-dealeron) (403 companies)
- [Companies using VinSolutions](./leads/companies-using-vinsolutions) (160 companies)
- [Companies using DealerInspire](./leads/companies-using-dealerinspire) (409 companies)
- [Companies using Roadster](./leads/companies-using-roadster) (24 companies)
- [Companies using Edmunds](./leads/companies-using-edmunds) (201 companies)
- [Companies using AutoTrader CA](./leads/companies-using-autotrader-ca) (67 companies)
- [Companies using AutoLeadStar](./leads/companies-using-autoleadstar) (69 companies)

### Backend & API Development
- [Companies using PHP](./leads/companies-using-php) (2,764,703 companies)
- [Companies using Python](./leads/companies-using-python) (141,659 companies)
- [Companies using Laravel](./leads/companies-using-laravel) (4,679 companies)
- [Companies using Django](./leads/companies-using-django) (940 companies)
- [Companies using Ruby on Rails](./leads/companies-using-ruby-on-rails) (8,710 companies)
- [Companies using Express](./leads/companies-using-express) (3,207,826 companies)
- [Companies using Firebase](./leads/companies-using-firebase) (8,602 companies)
- [Companies using ASP.NET Core](./leads/companies-using-asp.net-core) (10,119 companies)
- [Companies using Amazon API Gateway](./leads/companies-using-amazon-api-gateway) (870 companies)

### Blog
- [Companies using Medium](./leads/companies-using-medium) (30,873 companies)
- [Companies using Ghost](./leads/companies-using-ghost) (767 companies)
- [Companies using Blogger](./leads/companies-using-blogger) (4,199 companies)
- [Companies using Tumblr](./leads/companies-using-tumblr) (46,081 companies)
- [Companies using Hasnode](./leads/companies-using-hasnode) (340 companies)
- [Companies using Hatena Blog](./leads/companies-using-hatena-blog) (9 companies)
- [Companies using Hexo](./leads/companies-using-hexo) (14,204 companies)
- [Companies using Publii](./leads/companies-using-publii) (383 companies)
- [Companies using Micro.blog](./leads/companies-using-micro.blog) (332 companies)

### Booking & Calendar
- [Companies using Calendly](./leads/companies-using-calendly) (138,785 companies)
- [Companies using OpenTable](./leads/companies-using-opentable) (6,659 companies)
- [Companies using MindBody](./leads/companies-using-mindbody) (4,040 companies)
- [Companies using Setmore](./leads/companies-using-setmore) (539 companies)
- [Companies using YouCanBookMe](./leads/companies-using-youcanbookme) (1,206 companies)
- [Companies using Bookly](./leads/companies-using-bookly) (3,831 companies)
- [Companies using Vagaro](./leads/companies-using-vagaro) (7,434 companies)
- [Companies using Resy](./leads/companies-using-resy) (3,829 companies)
- [Companies using ZocDoc](./leads/companies-using-zocdoc) (2,888 companies)

### Business Intelligence
- [Companies using Google Looker Studio](./leads/companies-using-google-looker-studio) (259 companies)
- [Companies using Gainsight](./leads/companies-using-gainsight) (36 companies)
- [Companies using Chartbeat](./leads/companies-using-chartbeat) (612 companies)
- [Companies using ThoughtMetric](./leads/companies-using-thoughtmetric) (86 companies)

### Cloud Providers
- [Companies using Amazon](./leads/companies-using-amazon) (2,252,013 companies)
- [Companies using Google Cloud](./leads/companies-using-google-cloud) (4,981,407 companies)
- [Companies using Microsoft Azure](./leads/companies-using-microsoft-azure) (130,446 companies)
- [Companies using Heroku](./leads/companies-using-heroku) (42,027 companies)
- [Companies using Netlify](./leads/companies-using-netlify) (128,510 companies)
- [Companies using Vercel](./leads/companies-using-vercel) (324,738 companies)
- [Companies using Cloudflare Workers](./leads/companies-using-cloudflare-workers) (1,213 companies)
- [Companies using Rackspace](./leads/companies-using-rackspace) (7,056 companies)

### Content Management System (CMS)
- [Companies using WordPress](./leads/companies-using-wordpress) (4,113,853 companies)
- [Companies using Drupal](./leads/companies-using-drupal) (23,957 companies)
- [Companies using Joomla!](./leads/companies-using-joomla!) (33,643 companies)
- [Companies using Adobe Experience Manager](./leads/companies-using-adobe-experience-manager) (1,724 companies)
- [Companies using Sitecore CMS](./leads/companies-using-sitecore-cms) (1,524 companies)
- [Companies using Webflow](./leads/companies-using-webflow) (46,683 companies)
- [Companies using Squarespace](./leads/companies-using-squarespace) (1,157,536 companies)
- [Companies using Wix](./leads/companies-using-wix) (1,238,956 companies)

### Collaboration & Communication
- [Companies using Slack](./leads/companies-using-slack) (4,208 companies)
- [Companies using Microsoft Teams](./leads/companies-using-microsoft-teams) (1,776 companies)
- [Companies using Zoom](./leads/companies-using-zoom) (15,962 companies)
- [Companies using Discord](./leads/companies-using-discord) (12,493 companies)
- [Companies using Google Workspace](./leads/companies-using-google-workspace) (15,863 companies)
- [Companies using Microsoft 365](./leads/companies-using-microsoft-365) (25,992 companies)
- [Companies using Notion](./leads/companies-using-notion) (3,111 companies)
- [Companies using Figma](./leads/companies-using-figma) (8,091 companies)
- [Companies using Telegram](./leads/companies-using-telegram) (101,852 companies)
- [Companies using Workplace from Meta](./leads/companies-using-workplace-from-meta) (291 companies)

### Compliance & Governance
- [Companies using OneTrust](./leads/companies-using-onetrust) (16,104 companies)
- [Companies using CookieYes](./leads/companies-using-cookieyes) (46,840 companies)
- [Companies using Didomi](./leads/companies-using-didomi) (880 companies)
- [Companies using Trustarc Cookie Consent](./leads/companies-using-trustarc-cookie-consent) (1,663 companies)
- [Companies using Usercentrics](./leads/companies-using-usercentrics) (4,681 companies)
- [Companies using Termly](./leads/companies-using-termly) (9,100 companies)

### Content Delivery Networks (CDN)
- [Companies using Cloudflare](./leads/companies-using-cloudflare) (2,343,410 companies)
- [Companies using Akamai](./leads/companies-using-akamai) (6,465 companies)
- [Companies using Fastly](./leads/companies-using-fastly) (1,793,402 companies)
- [Companies using Amazon CloudFront](./leads/companies-using-amazon-cloudfront) (485,948 companies)
- [Companies using Cloudinary](./leads/companies-using-cloudinary) (39,792 companies)
- [Companies using StackPath](./leads/companies-using-stackpath) (60,102 companies)
- [Companies using BunnyCDN](./leads/companies-using-bunnycdn) (49,768 companies)
- [Companies using KeyCDN](./leads/companies-using-keycdn) (16,505 companies)
- [Companies using jsDelivr](./leads/companies-using-jsdelivr) (591,081 companies)
- [Companies using CacheFly](./leads/companies-using-cachefly) (178 companies)

### Conversion Tracking & Retargeting
- [Companies using Facebook Pixel](./leads/companies-using-facebook-pixel) (405,591 companies)
- [Companies using LinkedIn Insights](./leads/companies-using-linkedin-insights) (43,640 companies)
- [Companies using TikTok Conversion Tracking Pixel](./leads/companies-using-tiktok-conversion-tracking-pixel) (16,759 companies)
- [Companies using Pinterest Conversion Tracking](./leads/companies-using-pinterest-conversion-tracking) (10,678 companies)
- [Companies using Reddit Conversion Tracking](./leads/companies-using-reddit-conversion-tracking) (8,239 companies)
- [Companies using Kwai Conversion Tracking](./leads/companies-using-kwai-conversion-tracking) (47 companies)
- [Companies using PixelYourSite](./leads/companies-using-pixelyoursite) (22,411 companies)
- [Companies using Zemanta](./leads/companies-using-zemanta) (383 companies)

### Customer Relationship Management (CRM)
- [Companies using Salesforce](./leads/companies-using-salesforce) (14,059 companies)
- [Companies using Hubspot](./leads/companies-using-hubspot) (90,411 companies)
- [Companies using Pipedrive](./leads/companies-using-pipedrive) (2,364 companies)
- [Companies using Zoho CRM](./leads/companies-using-zoho-crm) (1,277 companies)
- [Companies using Active Campaign](./leads/companies-using-active-campaign) (5,668 companies)
- [Companies using Monday.com](./leads/companies-using-monday.com) (4,723 companies)
- [Companies using Freshsales](./leads/companies-using-freshsales) (111 companies)
- [Companies using CapsuleCRM](./leads/companies-using-capsulecrm) (66 companies)

### CSS Frameworks
- [Companies using Tailwind CSS](./leads/companies-using-tailwind-css) (2,273,121 companies)
- [Companies using Bootstrap](./leads/companies-using-bootstrap) (5,345 companies)
- [Companies using Material-UI](./leads/companies-using-material-ui) (24,298 companies)
- [Companies using Chakra UI](./leads/companies-using-chakra-ui) (1,952 companies)
- [Companies using Bulma](./leads/companies-using-bulma) (324,568 companies)
- [Companies using Foundation](./leads/companies-using-foundation) (16,161 companies)
- [Companies using Semantic UI](./leads/companies-using-semantic-ui) (1,039 companies)
- [Companies using Styled Components](./leads/companies-using-styled-components) (38,194 companies)
- [Companies using Radix UI](./leads/companies-using-radix-ui) (37,950 companies)
- [Companies using Mantine](./leads/companies-using-mantine) (2,798 companies)

### Customer Data Platforms (CDP)
- [Companies using Tealium](./leads/companies-using-tealium) (6,321 companies)
- [Companies using Blueshift](./leads/companies-using-blueshift) (58 companies)
- [Companies using Lytics](./leads/companies-using-lytics) (41 companies)
- [Companies using Treasure Data](./leads/companies-using-treasure-data) (97 companies)
- [Companies using Optimove](./leads/companies-using-optimove) (23 companies)

### Customer Support
- [Companies using Freshdesk Support](./leads/companies-using-freshdesk-support) (2,565 companies)
- [Companies using Help Scout](./leads/companies-using-help-scout) (1,797 companies)
- [Companies using Drift](./leads/companies-using-drift) (2,374 companies)
- [Companies using Gorgias](./leads/companies-using-gorgias) (5,810 companies)
- [Companies using LiveChat](./leads/companies-using-livechat) (7,375 companies)
- [Companies using Tidio](./leads/companies-using-tidio) (20,858 companies)
- [Companies using Crisp](./leads/companies-using-crisp) (6,891 companies)

### Cybersecurity
- [Companies using Auth0](./leads/companies-using-auth0) (929 companies)
- [Companies using Okta](./leads/companies-using-okta) (515 companies)
- [Companies using 1Password](./leads/companies-using-1password) (14,484 companies)
- [Companies using Cloudflare Turnstile](./leads/companies-using-cloudflare-turnstile) (30,351 companies)
- [Companies using reCAPTCHA](./leads/companies-using-recaptcha) (1,064,392 companies)
- [Companies using hCaptcha](./leads/companies-using-hcaptcha) (357,299 companies)
- [Companies using Wordfence](./leads/companies-using-wordfence) (541 companies)
- [Companies using Ping Identity](./leads/companies-using-ping-identity) (107 companies)
- [Companies using Bitwarden Passwordless](./leads/companies-using-bitwarden-passwordless) (719 companies)

### DNS Providers
- [Companies using Amazon Route 53](./leads/companies-using-amazon-route-53) (90,648 companies)
- [Companies using Namecheap](./leads/companies-using-namecheap) (18 companies)
- [Companies using Dynadot](./leads/companies-using-dynadot) (818 companies)
- [Companies using PointDNS](./leads/companies-using-pointdns) (30 companies)
- [Companies using Zonomi](./leads/companies-using-zonomi) (28 companies)

### Document Management
- [Companies using DocuSign](./leads/companies-using-docusign) (643 companies)
- [Companies using PandaDoc](./leads/companies-using-pandadoc) (101 companies)
- [Companies using RightSignature](./leads/companies-using-rightsignature) (90 companies)
- [Companies using Signaturit](./leads/companies-using-signaturit) (2 companies)
- [Companies using Foxit](./leads/companies-using-foxit) (27 companies)

### Ecommerce
- [Companies using Shopify](./leads/companies-using-shopify) (362,101 companies)
- [Companies using WooCommerce](./leads/companies-using-woocommerce) (434,708 companies)
- [Companies using Magento](./leads/companies-using-magento) (862,027 companies)
- [Companies using BigCommerce](./leads/companies-using-bigcommerce) (4,599 companies)
- [Companies using Salesforce Commerce Cloud](./leads/companies-using-salesforce-commerce-cloud) (725 companies)
- [Companies using Ecwid](./leads/companies-using-ecwid) (32,459 companies)
- [Companies using PrestaShop](./leads/companies-using-prestashop) (11,093 companies)
- [Companies using VTEX](./leads/companies-using-vtex) (58 companies)
- [Companies using Volusion](./leads/companies-using-volusion) (114 companies)
- [Companies using OpenCart](./leads/companies-using-opencart) (1,887 companies)

### Ecommerce Loyalty & Rewards
- [Companies using Yotpo](./leads/companies-using-yotpo) (4,902 companies)
- [Companies using Smile.io](./leads/companies-using-smile.io) (4,557 companies)
- [Companies using Loyalty Lion](./leads/companies-using-loyalty-lion) (44 companies)
- [Companies using Stamped](./leads/companies-using-stamped) (4,974 companies)
- [Companies using Growsurf](./leads/companies-using-growsurf) (46 companies)
- [Companies using Okendo](./leads/companies-using-okendo) (1,970 companies)

### Education
- [Companies using Moodle](./leads/companies-using-moodle) (53 companies)
- [Companies using Kajabi](./leads/companies-using-kajabi) (2,453 companies)
- [Companies using LearnDash](./leads/companies-using-learndash) (3,692 companies)
- [Companies using TalentLMS](./leads/companies-using-talentlms) (322 companies)
- [Companies using Blackboard Open LMS](./leads/companies-using-blackboard-open-lms) (160 companies)
- [Companies using Docebo](./leads/companies-using-docebo) (23 companies)
- [Companies using LifterLMS](./leads/companies-using-lifterlms) (503 companies)

### Email Marketing
- [Companies using MailChimp](./leads/companies-using-mailchimp) (107,686 companies)
- [Companies using Klaviyo](./leads/companies-using-klaviyo) (80,444 companies)
- [Companies using Constant Contact](./leads/companies-using-constant-contact) (47,533 companies)
- [Companies using ConvertKit](./leads/companies-using-convertkit) (6,025 companies)
- [Companies using Brevo](./leads/companies-using-brevo) (7,200 companies)
- [Companies using GetResponse](./leads/companies-using-getresponse) (68,570 companies)
- [Companies using Campaign Monitor Widget](./leads/companies-using-campaign-monitor-widget) (48 companies)
- [Companies using Drip](./leads/companies-using-drip) (1,659 companies)
- [Companies using MailerLite](./leads/companies-using-mailerlite) (4,128 companies)
- [Companies using AWeber](./leads/companies-using-aweber) (1,366 companies)

### Financial Services
- [Companies using Mercury](./leads/companies-using-mercury) (310 companies)
- [Companies using Tipalti](./leads/companies-using-tipalti) (76 companies)
- [Companies using Broadridge](./leads/companies-using-broadridge) (154 companies)
- [Companies using eMoney](./leads/companies-using-emoney) (58 companies)
- [Companies using TradingView](./leads/companies-using-tradingview) (8,757 companies)
- [Companies using Guidewire](./leads/companies-using-guidewire) (16 companies)
- [Companies using Optimal Blue](./leads/companies-using-optimal-blue) (218 companies)
- [Companies using YCharts](./leads/companies-using-ycharts) (22 companies)

### Forms & Surveys
- [Companies using Typeform](./leads/companies-using-typeform) (14,988 companies)
- [Companies using JotForm](./leads/companies-using-jotform) (4,202 companies)
- [Companies using Google Forms](./leads/companies-using-google-forms) (59,942 companies)
- [Companies using Wufoo](./leads/companies-using-wufoo) (4,025 companies)
- [Companies using Formstack](./leads/companies-using-formstack) (2,959 companies)
- [Companies using WPForms](./leads/companies-using-wpforms) (184,189 companies)
- [Companies using Gravity Forms](./leads/companies-using-gravity-forms) (195,084 companies)
- [Companies using Paperform](./leads/companies-using-paperform) (1,129 companies)
- [Companies using Qualtrics](./leads/companies-using-qualtrics) (3,142 companies)

### Forum & Community
- [Companies using Discourse](./leads/companies-using-discourse) (12 companies)
- [Companies using bbPress](./leads/companies-using-bbpress) (8,485 companies)
- [Companies using BetterMode](./leads/companies-using-bettermode) (67 companies)
- [Companies using BuddyBoss](./leads/companies-using-buddyboss) (1,181 companies)
- [Companies using Invision Power Board](./leads/companies-using-invision-power-board) (13 companies)

### Headless CMS
- [Companies using Contentful](./leads/companies-using-contentful) (2,195 companies)
- [Companies using Sanity](./leads/companies-using-sanity) (13,940 companies)
- [Companies using Prismic](./leads/companies-using-prismic) (1,349 companies)
- [Companies using Contentstack](./leads/companies-using-contentstack) (1,961 companies)
- [Companies using DatoCMS](./leads/companies-using-datocms) (1,302 companies)
- [Companies using ButterCMS](./leads/companies-using-buttercms) (100 companies)

### Healthcare
- [Companies using AthenaHealth](./leads/companies-using-athenahealth) (2,033 companies)
- [Companies using ZocDoc](./leads/companies-using-zocdoc) (2,888 companies)
- [Companies using Practice Fusion](./leads/companies-using-practice-fusion) (64 companies)
- [Companies using Jane App](./leads/companies-using-jane-app) (248 companies)
- [Companies using Cliniko](./leads/companies-using-cliniko) (3,189 companies)
- [Companies using Practice Better](./leads/companies-using-practice-better) (1,877 companies)
- [Companies using TherapyNotes](./leads/companies-using-therapynotes) (225 companies)
- [Companies using Kareo](./leads/companies-using-kareo) (730 companies)
- [Companies using Luma Health](./leads/companies-using-luma-health) (25 companies)
- [Companies using NexHealth](./leads/companies-using-nexhealth) (761 companies)

### HR & Human Resources
- [Companies using Workday](./leads/companies-using-workday) (1,313 companies)
- [Companies using Gusto](./leads/companies-using-gusto) (790 companies)
- [Companies using Greenhouse](./leads/companies-using-greenhouse) (2,058 companies)
- [Companies using Lever](./leads/companies-using-lever) (589 companies)
- [Companies using Breezy HR](./leads/companies-using-breezy-hr) (1,512 companies)
- [Companies using Recruitee](./leads/companies-using-recruitee) (501 companies)
- [Companies using Freshteam](./leads/companies-using-freshteam) (89 companies)

### Integration & Automation
- [Companies using Merge](./leads/companies-using-merge) (25 companies)
- [Companies using Outfunnel](./leads/companies-using-outfunnel) (345 companies)
- [Companies using Pabbly](./leads/companies-using-pabbly) (184 companies)

### JavaScript Frameworks
- [Companies using React](./leads/companies-using-react) (13,328 companies)
- [Companies using Angular](./leads/companies-using-angular) (53,941 companies)
- [Companies using Next.js](./leads/companies-using-next.js) (459,465 companies)
- [Companies using Nuxt.js](./leads/companies-using-nuxt.js) (145,549 companies)
- [Companies using jQuery](./leads/companies-using-jquery) (2,984,632 companies)
- [Companies using Astro](./leads/companies-using-astro) (39,528 companies)

### Lead Generation
- [Companies using Unbounce](./leads/companies-using-unbounce) (164 companies)
- [Companies using Leadpages](./leads/companies-using-leadpages) (97 companies)
- [Companies using ClickFunnels](./leads/companies-using-clickfunnels) (1,195 companies)
- [Companies using OptinMonster](./leads/companies-using-optinmonster) (3,670 companies)
- [Companies using Sumo](./leads/companies-using-sumo) (1,219 companies)

### Marketing
- [Companies using Marketo](./leads/companies-using-marketo) (1,525 companies)
- [Companies using Pardot](./leads/companies-using-pardot) (2,623 companies)
- [Companies using SharpSpring](./leads/companies-using-sharpspring) (994 companies)
- [Companies using Iterable](./leads/companies-using-iterable) (572 companies)
- [Companies using Mautic](./leads/companies-using-mautic) (853 companies)
- [Companies using Netcore](./leads/companies-using-netcore) (12 companies)

### Monitoring & Logging
- [Companies using New Relic](./leads/companies-using-new-relic) (28,082 companies)
- [Companies using LogRocket](./leads/companies-using-logrocket) (591 companies)
- [Companies using Grafana](./leads/companies-using-grafana) (90 companies)
- [Companies using Better Stack](./leads/companies-using-better-stack) (170 companies)

### Non-Profit
- [Companies using Blackbaud](./leads/companies-using-blackbaud) (891 companies)
- [Companies using Classy](./leads/companies-using-classy) (1,672 companies)
- [Companies using Givebutter](./leads/companies-using-givebutter) (9,133 companies)
- [Companies using Donorbox](./leads/companies-using-donorbox) (4,921 companies)
- [Companies using Kindful](./leads/companies-using-kindful) (1,088 companies)
- [Companies using GoFundMe](./leads/companies-using-gofundme) (5,271 companies)

### Performance Optimization
- [Companies using EWWW Image Optimizer](./leads/companies-using-ewww-image-optimizer) (15,600 companies)
- [Companies using Smush Image Optimization](./leads/companies-using-smush-image-optimization) (36,480 companies)
- [Companies using LiteSpeed](./leads/companies-using-litespeed) (1,057,756 companies)

### Popups & Modals
- [Companies using Elfsight](./leads/companies-using-elfsight) (74,896 companies)
- [Companies using Commoninja](./leads/companies-using-commoninja) (16,582 companies)
- [Companies using OptiMonk](./leads/companies-using-optimonk) (3,056 companies)
- [Companies using Sleeknote](./leads/companies-using-sleeknote) (544 companies)
- [Companies using WisePops](./leads/companies-using-wisepops) (986 companies)
- [Companies using Poptin](./leads/companies-using-poptin) (6,742 companies)
- [Companies using ConvertBox](./leads/companies-using-convertbox) (1,121 companies)
- [Companies using ConvertFlow](./leads/companies-using-convertflow) (970 companies)
- [Companies using Hello Bar](./leads/companies-using-hello-bar) (1,434 companies)
- [Companies using Privy](./leads/companies-using-privy) (7,859 companies)
- [Companies using Boxzilla](./leads/companies-using-boxzilla) (1,231 companies)

### Project Management
- [Companies using Asana](./leads/companies-using-asana) (947 companies)
- [Companies using Trello](./leads/companies-using-trello) (996 companies)
- [Companies using JIRA](./leads/companies-using-jira) (1,709 companies)
- [Companies using Monday.com](./leads/companies-using-monday.com) (4,723 companies)
- [Companies using ClickUp](./leads/companies-using-clickup) (858 companies)
- [Companies using Wrike](./leads/companies-using-wrike) (107 companies)
- [Companies using Smartsheet](./leads/companies-using-smartsheet) (615 companies)
- [Companies using Todoist](./leads/companies-using-todoist) (60 companies)

### Push Notifications & User Engagement
- [Companies using OneSignal](./leads/companies-using-onesignal) (4,640 companies)
- [Companies using Pusher](./leads/companies-using-pusher) (1,712 companies)
- [Companies using PushEngage](./leads/companies-using-pushengage) (437 companies)
- [Companies using Airship](./leads/companies-using-airship) (162 companies)
- [Companies using Iterable](./leads/companies-using-iterable) (572 companies)
- [Companies using MoEngage](./leads/companies-using-moengage) (1,693 companies)

### Real Estate
- [Companies using Yardi](./leads/companies-using-yardi) (67 companies)
- [Companies using RealPage](./leads/companies-using-realpage) (4,445 companies)
- [Companies using BoomTown](./leads/companies-using-boomtown) (103 companies)
- [Companies using CINC](./leads/companies-using-cinc) (1,119 companies)
- [Companies using Follow Up Boss](./leads/companies-using-follow-up-boss) (561 companies)
- [Companies using Propertybase](./leads/companies-using-propertybase) (164 companies)
- [Companies using Placester](./leads/companies-using-placester) (2,359 companies)

### Sales Enablement
- [Companies using Seismic](./leads/companies-using-seismic) (104 companies)
- [Companies using Highspot](./leads/companies-using-highspot) (54 companies)
- [Companies using Outreach](./leads/companies-using-outreach) (1,840 companies)
- [Companies using SalesLoft](./leads/companies-using-salesloft) (943 companies)
- [Companies using PandaDoc](./leads/companies-using-pandadoc) (101 companies)

### Search & Indexing
- [Companies using Algolia](./leads/companies-using-algolia) (2,499 companies)
- [Companies using Swiftype](./leads/companies-using-swiftype) (87 companies)
- [Companies using Coveo](./leads/companies-using-coveo) (203 companies)
- [Companies using SearchSpring](./leads/companies-using-searchspring) (259 companies)
- [Companies using Doofinder](./leads/companies-using-doofinder) (1,485 companies)
- [Companies using Site Search 360](./leads/companies-using-site-search-360) (1,001 companies)
- [Companies using Sajari](./leads/companies-using-sajari) (21 companies)

### Shipping & Logistics
- [Companies using UPS](./leads/companies-using-ups) (7,362 companies)
- [Companies using FedEx](./leads/companies-using-fedex) (899 companies)
- [Companies using USPS](./leads/companies-using-usps) (1,106 companies)
- [Companies using DHL](./leads/companies-using-dhl) (704 companies)
- [Companies using ShipStation](./leads/companies-using-shipstation) (121 companies)
- [Companies using Route](./leads/companies-using-route) (93,859 companies)
- [Companies using Narvar](./leads/companies-using-narvar) (101 companies)
- [Companies using Deliverr](./leads/companies-using-deliverr) (180 companies)

### Social Media Management
- [Companies using Buffer](./leads/companies-using-buffer) (201 companies)
- [Companies using Hootsuite](./leads/companies-using-hootsuite) (3,636 companies)
- [Companies using Meet Edgar](./leads/companies-using-meet-edgar) (14 companies)
- [Companies using Sendible](./leads/companies-using-sendible) (7 companies)

### Static Site Generator
- [Companies using Jekyll](./leads/companies-using-jekyll) (14,940 companies)
- [Companies using Gatsby JS](./leads/companies-using-gatsby-js) (8,827 companies)
- [Companies using Docusaurus](./leads/companies-using-docusaurus) (661 companies)
- [Companies using Gridsome](./leads/companies-using-gridsome) (646 companies)
- [Companies using Scully](./leads/companies-using-scully) (1,398 companies)

### Video Platforms
- [Companies using YouTube](./leads/companies-using-youtube) (1,859,149 companies)
- [Companies using Vimeo](./leads/companies-using-vimeo) (238,231 companies)
- [Companies using Wistia](./leads/companies-using-wistia) (11,467 companies)
- [Companies using Brightcove](./leads/companies-using-brightcove) (1,487 companies)
- [Companies using Kaltura](./leads/companies-using-kaltura) (490 companies)
- [Companies using JW Player](./leads/companies-using-jw-player) (850 companies)
- [Companies using Twitch](./leads/companies-using-twitch) (1,446 companies)
- [Companies using DailyMotion Video](./leads/companies-using-dailymotion-video) (3,115 companies)

### Web Fonts & Icons
- [Companies using Google Font API](./leads/companies-using-google-font-api) (4,554,496 companies)
- [Companies using Font Awesome](./leads/companies-using-font-awesome) (767,520 companies)
- [Companies using MyFonts](./leads/companies-using-myfonts) (3,120 companies)
- [Companies using Fontshare](./leads/companies-using-fontshare) (2,790 companies)
- [Companies using Bunny Fonts](./leads/companies-using-bunny-fonts) (41,445 companies)
- [Companies using Fontello](./leads/companies-using-fontello) (41,780 companies)
- [Companies using IcoMoon](./leads/companies-using-icomoon) (1,275,069 companies)
- [Companies using Material Design Icons](./leads/companies-using-material-design-icons) (23,476 companies)

### Web Servers
- [Companies using NGINX](./leads/companies-using-nginx) (1,363,296 companies)
- [Companies using Apache](./leads/companies-using-apache) (1,352,819 companies)
- [Companies using LiteSpeed](./leads/companies-using-litespeed) (1,057,756 companies)
- [Companies using OpenResty](./leads/companies-using-openresty) (222,398 companies)
- [Companies using Apache Tomcat](./leads/companies-using-apache-tomcat) (529 companies)


## 🚀 Quick Start

1. **Browse**: Click any folder above to explore companies
2. **Download**: Use the CSV/JSON files to export data
3. **Integrate**: Use the free data to analyze tech trends
4. **Upgrade**: Get verified contact info for any company list

## ✨ Features

- ✅ **Updated Daily**: Fresh data every 24 hours
- ✅ **Free to Use**: No registration required
- ✅ **Open Format**: CSV & JSON for easy integration
- ✅ **Verified Data**: Detected by our crawler, not scraped
- ✅ **Complete Records**: 10+ data fields per company
- ✅ **Downloadable**: Use offline or in your tools

## 📊 What's Included?

Each dataset contains:
- **Company Name** & Domain
- **Location** (Country, State)
- **Technology Stack** (detected tools & platforms)
- **Spending Score** (estimated tech budget)
- **Quality Score** (data verification score)
- **Service Type** (B2B, B2C, etc)
- **Decision-Maker Title** (when available)
- **Last Verified** (crawl date)

## 🔐 Contact Information

The datasets above show sample records **without emails or phone numbers**.

To unlock verified contact information for any company list:
👉 [Upgrade to Leadita Pro](https://leadita.com)

## 📈 Statistics

- **Total Companies Indexed**: 55.5M+
- **Total Technologies Tracked**: 403
- **Data Update Frequency**: Daily
- **Last Mass Update**: 2026-09-26

## 🛠️ Use Cases

**Sales & Outreach**
- Build prospect lists by technology
- Find companies to pitch your product
- Identify decision-makers

**Marketing & Competitive Intelligence**
- Track competitor tech stacks
- Understand market adoption trends
- Benchmark against similar companies

**Product Teams**
- Analyze market adoption of your tools
- Find early adopters of new technologies
- Track tech trends over time

**Data Scientists & Analysts**
- Train ML models on tech adoption patterns
- Analyze market segmentation
- Study digital transformation trends

## 💡 API Access

Want to integrate this data programmatically? [Check out Leadita API](https://leadita.com)

## 📝 License

This dataset is provided as-is for informational purposes. Use of contact information requires explicit consent. See [LICENSE](./LICENSE) for details.

## 🤝 Contributing

Found an issue with the data? Have suggestions? [Open an issue](https://github.com/your-username/tech-stack-datasets/issues)

## 📧 Support

Questions? Feedback? [Contact us](https://leadita.com/contact-sales)

---

**Made with ❤️ by [Leadita](https://leadita.com)**
